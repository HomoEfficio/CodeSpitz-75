...
*참고: 강의 자료에는 `DataSupply`라고 표시되어 있는 부분인데, 개인적인 의견으로는 이와 같이 순수하게 데이터라는 의도라면 `DataSupply`보다 `DataSource`라는 이름이 더 적절할 것 같다. `Supplier`라고 하면 데이터를 공급해 줄 대상을 알고, 그 대상에게 직접 데이터를 공급해주는 역할이 있을 거라고 짐작하게 되는데, 여기에서의 Data는 실제로는 그런 역할 없이 그냥 데이터 그 자체이기 때문이다.*
